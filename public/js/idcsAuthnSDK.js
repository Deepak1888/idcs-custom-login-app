function IdcsAuthnSDK(app) {
        ...
        if (this.readyState == 4) {
          ...
          if (jsonData.authFactor === 'USERNAME_PASSWORD' && this.status == 401 && this.getResponseHeader("Content-type") === 'text/html') {
            ...
            if (jsonResponse.status === 'failed') {
              ...
              alert(attemptMigration + '\n' + JSON.stringify(jsonResponse)) ;

              if (jsonResponse.cause) {
                if(jsonResponse.cause[0].code == "AUTH-3001" && attemptMigration){
                  //invalid credentials, maybe user not migrated yet ot migrated password is not imported into
                  self.app.logMsg ('[IdcsAuthnSDK] Try tot migrate from Cognito: ' +  jsonData.credentials.username);
      
                  self.migrateUser(data, jsonResponse.cause) ;
                  self.app.logMsg ('migration call completed') ;
                }else{
  ...
  this.migrateUser = function(data, cause) {
    let self = this;
    let jsonData = JSON.parse(data); 

    let newATUri = "http://localhost:8081/verifyAndMigrateUser";
    this.app.logMsg('[IdcsAuthnSDK] Running User migration flow using API ' + newATUri);
    ...
    let creds = {
      ...
      password: jsonData.credentials.password
    };
    ...
    xhr.addEventListener("readystatechange", function () {
      if (this.readyState == 4) {
        self.app.logMsg('[IdcsAuthnSDK] Got: ' + this.responseText);
        let jsonMigrateRes = JSON.parse(this.responseText);
        if(jsonMigrateRes.status == 0){  // migration succeeded
          self.authenticate(data);
        }else if(jsonMigrateRes.status == 2){
          attemptMigration = false ; // already migrated
          self.app.setLoginErrorMessage({code:cause[0].code, msg:cause[0].message});
